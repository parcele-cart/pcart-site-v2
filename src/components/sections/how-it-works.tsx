    ...
    >
      ...
      <motion.div
        ...
        className={cn(
          ...
          isActive ? "ring-2 ring-brand-green/50 border-brand-green/30 bg-gradient-to-br from-brand-green/20 via-brand-green/5 to-transparent shadow-[0_0_50px_-12px_rgba(94,242,117,0.4)] scale-[1.05]" : 
          isPast ? "opacity-60 grayscale-[0.5] border-transparent" : "opacity-40 border-transparent"
        )}
      >
        {/* Glow behind icon */}
        {isActive && (
        ...
        <div className="relative inline-flex items-center justify-center w-16 h-16 lg:w-20 lg:h-20 rounded-full mb-6 z-10 transition-all duration-500">
          <motion.div 
            animate={{
              backgroundColor: isActive ? "var(--brand-green)" : "rgba(94, 242, 117, 0.1)",
              boxShadow: isActive ? "0 0 30px rgba(94, 242, 117, 0.6)" : "none",
              scale: isActive ? 1.1 : 1
            }}
            className="absolute inset-0 rounded-full border border-brand-green/20"
          />
          {/* Icon - Fixed color visibility */}
          <step.icon className={cn(
            "w-7 h-7 lg:w-8 lg:h-8 transition-colors duration-500 z-20",
            isActive ? "text-[#0E182A]" : "text-brand-green" // Forced dark color on active neon background
          )} />
          <span className={cn(
            ...
            isActive ? "text-brand-green border-brand-green scale-110" : "text-brand-green/60"
          )}>
            {step.number}
          </span>
        </div>
...
export function HowItWorks() {
  ...
  // Reduced scroll region to ensure activation happens only when well within viewport
  ...
  useMotionValueEvent(smoothProgress, "change", (latest) => {
    // Map progress to card indices (0, 1, 2) with tighter thresholds
    if (latest < 0.15) setActiveIndex(-1)
    else if (latest < 0.45) setActiveIndex(0)
    else if (latest < 0.75) setActiveIndex(1)
    else if (latest < 0.95) setActiveIndex(2)
    else setActiveIndex(-1) // Deactivate when section is mostly scrolled past
  })
        ...
        <div className="text-center max-w-4xl mx-auto mb-16 lg:mb-20">
          ...
          <BlurFade delay={0.1} inView>
            <h2 className="font-display text-3xl sm:text-4xl md:text-5xl lg:text-6xl text-foreground dark:text-white mb-6 tracking-tight">
              Três passos para <span className="text-brand-green">simplificar</span> tudo
            </h2>
          </BlurFade>
